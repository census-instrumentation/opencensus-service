...
func TestMultiAndQueuedSpanProcessorConfig(t *testing.T) {
	...
	for i := range wCfg.Processors {
		if !reflect.DeepEqual(*wCfg.Processors[i], *gCfg.Processors[i]) {
			...
			testutils.SaveAsFormattedJSON(fmt.Sprintf("./testdata/failure_queued_processor_cfg_got_%02d.json", i), *gCfg.Processors[i])
			testutils.SaveAsFormattedJSON(fmt.Sprintf("./testdata/failure_queued_processor_cfg_want_%02d.json", i), *wCfg.Processors[i])
		}
	}
}
