...
func (sp *queuedSpanProcessor) ProcessSpans(ctx context.Context, td data.TraceData) error {
	...
	statsTags := processor.StatsTagsForBatch(sp.name, processor.ServiceNameForNode(td.Node))
...
func (sp *queuedSpanProcessor) processItemFromQueue(item *queueItem) {
	...
	if err == nil {
		...
		statsTags := processor.StatsTagsForBatch(sp.name, processor.ServiceNameForNode(item.td.Node))
	...
	// There was an error
	statsTags := processor.StatsTagsForBatch(sp.name, processor.ServiceNameForNode(item.td.Node))
	stats.RecordWithTags(item.ctx, statsTags, statFailedSendOps.M(1))
...
func (sp *queuedSpanProcessor) onItemDropped(item *queueItem, statsTags []tag.Mutator) {
	...
	stats.RecordWithTags(item.ctx, statsTags, processor.StatDroppedSpanCount.M(int64(numSpans)))
